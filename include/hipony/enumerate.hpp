...
HIPONY_ENUMERATE_CONSTEXPR auto const dynamic_extent = -1;

template<typename T, typename Size, Size N = detail::dynamic_extent>
class span {
public:
    using value_type      = T;
    using pointer         = T*;
...
template<typename Size, typename T>
struct dispatch<detail::pointer_tag_t, Size, T> {
